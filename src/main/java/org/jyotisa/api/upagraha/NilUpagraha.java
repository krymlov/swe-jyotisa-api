...
/**
 * The "not a upagraha" member - a Null Object, so that a lookup which cannot name one has
 ...
 * the <b>contract</b> of IUpagraha rather than of any one implementation: a caller holding only the
 * interface can test the result with {@link org.swisseph.api.ISweEnum#isNil()}.
 * <p>
 * Before this existed, {@code EUpagraha.NIL.upagraha()} answered {@code null} - so every {@code by*}
 ...
 * <p>
 * Its accessors stay {@code null}: a non-upagraha has no properties, and inventing some would trade
 * a visible failure for an invisible wrong answer - which is what this class exists to stop.
 *
 * @author Yura Krymlov
 * @version 1.0, 2026-08
 */
public enum NilUpagraha implements IUpagraha {
    NIL;
